...
The C65 includes another facility for making C64 ROM regions visible in C65
mode, as well as controlling the visibility of colour memory in the I/O
register region. These are banked in and out using various bits of the
VIC-III's \$D030 register:
...
The first 2KB of colour RAM is always visible at \$1F800 -- \$1FFFF. The first
1KB of this is also visible at \$D800 -- \$DBFF. When the CRAM2K signal is 1,
the second kilobyte is made available at \$DC00 -- \$DFFF. Unlike on the C64,
the colour RAM on the MEGA65 is always visible as 8-bit bytes.  Also, on the
MEGA65, the colour RAM is 32KB in size, and exists at \$FF80000 -- \$FF87FFF.
The visibility of the colour RAM at \$1F800 -- \$1FFFF is achieved by
mirroring writes to both regions when accessing the colour RAM via this
mechanism.

Both VIC-III banking and C64-style banking can apply to \$A000 -- \$BFFF and
\$E000 -- \$FFFF. They have the same effect of mapping those regions to ROM
addresses in bank 2. If either mechanism is active for a region, then the
region is mapped to ROM.

VIC ROM banking accesses the C64 ROM only. The C65/MEGA65 ROM is accessed
using the MAP register.
...
processors internal registers that hold the mapping information.

The following diagram illustrates how the MAP instruction takes the values of the four A, X, Y and Z
registers, and uses them to compute the upper and lower address offsets, and sets the bank enable bits
...
As another example of how the MAP instruction can be used to map an area of memory from
...
is running in the bottom 64KB of RAM, and not in the region between \$6000 -- \$8000.
...
\item{\bf Certain keyboard events} Pressing \widekey{RESTORE} for >0.5 seconds, or the \megasymbolkey and
\specialkey{TAB} key combination traps to the hypervisor.  Typically the first is used to launch the Freeze Menu and the second to toggle the display of the debug interface.
