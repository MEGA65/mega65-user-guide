...
\stw{SBCQ}, \stw{CPQ}, \stw{EORQ}, \stw{ANDQ}, \stw{ORQ}, \stw{ASLQ}, \stw{LSRQ}, \stw{ROLQ}, \stw{RORQ}, \stw{INQ} and \stw{DEQ}
mnemonics.\index{LDQ}\index{STQ}\index{ADCQ}\index{SBCQ}\index{CPQ}\index{EORQ}\index{ANDQ}\index{ORQ}\index{ASLQ}\index{LSRQ}\index{ROLQ}\index{RORQ}\index{INQ}\index{DEQ} The previous example would thus become:
...
be used with care, because these registers may in fact be holding part of a 32-bit value.  The exception is the Zero-Page
...
avoid problems with variable length instructions. Also, it would save at most one byte
...
\section{MEGA65 CPU Maths Unit Registers}

Some models include a math unit, which helps to accelerate the calculation of fixed-point formulae.
This is not present in all models of the MEGA65.
