...
\item [Usage:] The decimal value
               of a hexadecimal string.

               The argument range is "0000" to "FFFF" (0 to 65535 in decimal).
               The argument must have 1-4 hex digits.

\item [Remarks:] Allowed digits in uppercase/graphics mode are 0-9 and A-Z (\screentext{0123456789ABCDEF})
                 and in lowercase/uppercase mode are 0-9 and a-z (\screentext{0123456789abcdef}).
...
\item [Usage:]  {\bf mask4} and {\bf mask5} are byte values, that
...
{\bf Range}  & {\bf Content} \\
\hline
0000 - 1FFF & bank 0 with direct page, stack, vectors \\
...
The RAM banks 0, 1, 4 and 5 may be used on a MEGA65 with the {\bf SYS} command.
The attic RAM cannot be used for this purpose, because the 24 bit address
of the {\bf SYS command} is limited to the lower 16MB of the address range.
...
\item [Remarks:] The register values after a {\bf SYS} call are stored
                 ...
                 well known {\bf SYS} command on the C64. It is not possible
                 to have the BASIC ROM and a BASIC program, in the same mapping
                 because they occupy the same address range.
