...
\item [Usage:]  Prints a file directory/catalog of the specified disk.
   ...
   {\bf filepattern} is either a quoted string: \screentext{"DA*"} \\
   Or a string expression in brackets: \screentext{(DI\$)}
...
\megakey{F9} or \megakey{F11}, edited, listed, saved or printed.
...
\item [Remarks:] After reserving memory with {\bf MEM} the graphics
                 ...
                 Some direct mode commands such as {\bf RENUMBER} use memory in
...
\item [Usage:]  Prints a series of values formatted using a pattern to the current output
                ...
                or in C style, using a {\bf width}.{\bf precision} specifier such as \screentext{\%3D \%7.2F \%4X }.
...
\item [Remarks:] The format string is applied for one argument only,
                ...
                With a C-style pattern, you can request left-padding with zeroes instead of spaces by preceding the {\bf width} with a \screentext{0}, such as: \screentext{\%07.2F} Left zero padding is cancelled if the value is negative. Right-of-decimal fields and hexadecimal patterns are always left-padded with zeroes.
...
\item [Usage:]  Parses the {\bf format} string and evaluates the argument.
                ...
                {\bf format} string variable or a string constant
                which defines the rules for formatting.
                When using a number as the {\bf argument}, the pattern can be specified as a template such as  \screentext{\#\#\#.\#\#}
                or using a \%[0]<width>.<precision> specifier, such as: \screentext{\%3D \%7.2F \%4X } When a width specifier has a leading zero, the pattern left-pads a positive number with zeroes to the left of the decimal point: \screentext{\%07.2F }
