...
\subsection{Fonts}
Finally, whenever there's a piece of text in this appendix that reflects some logic, something you can type,
...
String   Array &  3     &    \screentext{\$}    & length = 0 .. 255 & \screentext{AB\$(X) = "TEXT"} \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item {\bf Logical Operator Example}: \screentext{IF A>B AND A<0}
  \item {\bf Boolean Operator Example}: \screentext{A = B AND \$7F}
\end{itemize}
...
for example,
\screentext{A = A - (B > 7)}
will increment \screentext{A} by 1 if the result of \screentext{(B > 7)} is {\bf TRUE} (-1).
...
\hline
 =   & assignment & \screentext{A = 42, A\$ ="HELLO", A = B < 42}\\
\hline
\end{tabular}
\end{center}
...
\item [Remarks:] {\bf CIRCLE} is used to draw circles on
               ...
               \screentextwide{RND} for circle colour, size and position.
