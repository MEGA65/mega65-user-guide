...
\item [Usage:]    Reserves memory in banks 4 or 5 such that the bitmap graphics system will not use it.
                  ...
                  {\bf mask4} and {\bf mask5} are byte values, interpreted as a bitfield. Each bit set to 1 reserves an 8KB segment of memory in bank 4 for the first argument and in bank 5 for the second argument.
                  \texttt{
                  \setlength{\tabcolsep}{1mm}
                  \begin{center}
                  \begin{tabular}{|l|l|}
...
\item [Remarks:]  After reserving memory with {\bf MEM}, the graphics library will not use the reserved areas, so it can be used for other purposes. Access to bank 4 and 5 is possible with the commands {\bf PEEK, WPEEK, POKE, WPOKE} and {\bf EDMA}.
                  ...
                  When 80 $\times$ 50 text mode is enabled, segment 0 of bank 4 is reserved automatically and used for screen data. It always uses segment 0, even if it was previously reserved with {\bf MEM} or a graphic screen. If your program uses 80 $\times$ 50 text mode and also reserves a region with {\bf MEM}, be sure to set region 0 of bank 4 as reserved, and do not use it for other purposes.
