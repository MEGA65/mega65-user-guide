...
\item [Usage:] Change to a subdirectory or a parent directory
   ...
   {\bf CHDIR ".."} changes to the parent directory.

   \filenamedefinition

   The {\bf,U} unit argument is currently ignored.
   {\bf CHDIR} can only be used on unit 12, the SD-card.

\item [Examples:] Using {\bf CHDIR}
\begin{tcolorbox}[colback=black,coltext=white]
\verbatimfont{\codefont}
\begin{verbatim}
  CHDIR "ADVENTURES"      :REM ENTER SUBDIRECTORY ADVENTURES
  CHDIR ".."              :REM GO BACK TO PARENT DIRECTORY
  CHDIR "RACING"          :REM ENTER SUBDIRECTORY RACING
\end{verbatim}
\end{tcolorbox}
\end{description}
...
\item [Usage:]  Clears (resets) a single bit at the {\bf address}.
                ...
                The {\bf bit number} ia a value in the range of 0-7.
...
\item [Format:] {\bf COLOR} colour\\
                {\bf COLOR} <{\bf ON} | {\bf OFF}>\\
\item [Usage:] The first format works in the same way as the {\bf FOREGROUND}, i.e: sets the foreground colour
               (text colour) of the screen to the colour argument, which must be in the range of 0 to 31. Refer to the table
               under {\bf BACKGROUND} on page \pageref{colourtable} for the colour values and their corresponding colours.

               The second format enables or disables
...
\item [Remarks:] After reserving memory with {\bf MEM} the graphics
                 ...
                 is possible with the commands {\bf PEEK, PEEKW, POKE, POKEW}
                 and {\bf EDMA}.
