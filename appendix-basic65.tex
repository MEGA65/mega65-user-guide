...
\item [Format:] {\bf COLOR} <{\bf ON} | {\bf OFF}>\\
                {\bf COLOR} colour
\item [Usage:] The first format enables or disables
               ...
               The second format works in the same way as the {\bf FOREGROUND}, i.e: sets the foreground colour
               (text colour) of the screen to the colour argument, which must be in the range of 0 to 31. Refer to the table
               under {\bf BACKGROUND} on \pageref{colourtable} for the colour values and their corresponding colours.
\item
\item which is documented
               on page \pageref{foregroundcommand}.
...
\item [Usage:]  {\bf EDIT} switches the built-in editor
               ...
               or ot the BASIC program editor with{\bf EDIT OFF}.
               ...
               The text mode affects entered lines with leading numbers only,
...
\item [Usage:]
   ...
   {\bf sub} sub command (see chapter on DMA controller in the MEGA65 Complete Compendium).

   {\bf mod} modifier (see chapter on DMA controller in the MEGA65 Complete Compendium).
...
\item [Remarks:] {\bf ERASE} works similarly to {\bf SCRATCH}.
...
\item [Remarks:] {\bf FOREGROUND} is a synonym of {\bf COLOR}.
...
\item [Remarks:] The sum of the lengths of all assigned strings
                 ...
                 using their codes with the {\bf CHR\$} command.
