...
by giving a {\ttfamily ?SYNTAX ERROR} message.
...
This could be a string ({\ttfamily "HELLO"}), a number
({\ttfamily 23.7}), or a calculation that might include
one or more functions or operators ({\ttfamily LEN("HELLO") * (3 XOR 7)}).
...
For example, {\ttfamily "HELLO"} is a {\bf string expression}, while
{\ttfamily 23.7} is a {\bf numeric expression}.
...
{\ttfamily ?TYPE MISMATCH ERROR}, to say that the type
of expression you gave doesn't match what it expected. For example, we will get a
{\ttfamily ?TYPE MISMATCH ERROR} if we type the following command,
because {\ttfamily "POTATO"} is a string expression instead of a numeric expression:
...
program, but can be entered directly to the screen.
...
\hline
Decimal Integer         &  \screentextwide{32000} & \screentextwide{-55}      \\
Decimal Fixed Point     &  \screentextwide{3.14}  & \screentextwide{-7654.321}\\
Decimal Floating Point  &  \screentextwide{1.5E03}& \screentextwide{7.7E-02}  \\
Hex                     &  \screentextwide{\$D020}& \screentextwide{\$FF}     \\
String                  &  \screentextwide{"X"}   & \screentextwide{"TEXT"}   \\
\hline
\end{tabular}
\end{center}
...
\hline
Byte                    & \screentextwide{\&} & 0 .. 255          & \screentextwide{BY\& = 23}    \\
Integer                 & \screentextwide{\%} & -32768 .. 32767   & \screentextwide{I\% = 5}      \\
Real                    & none                & -1E37 .. 1E37     & \screentextwide{XY = 1/3}     \\
String                  & \screentextwide{\$} & length = 0 .. 255 & \screentextwide{AB\$ = "TEXT"}\\
\hline
\end{tabular}
\end{center}
...
\hline
Byte     Array &  1     &    \screentextwide{\&}    & 0 .. 255          & \screentextwide{BY\&(5,6) = 23}   \\
Integer  Array &  2     &    \screentextwide{\%}    & -32768 .. 32767   & \screentextwide{I\%(0,10) = 5}    \\
Real     Array &  5     &   none                    & -1E37 .. 1E37     & \screentextwide{XY(I\%) = 1/3}    \\
String   Array &  3     &    \screentextwide{\$}    & length = 0 .. 255 & \screentextwide{AB\$(X) = "TEXT"} \\
\hline
\end{tabular}
\end{center}
...
programming languages. The usage and precedences of these operators is documented in this section.

The \screentextwide{=} symbol is used both as an assignment operator, and as a relational
...
while the second is a logical operator, comparing the variable \screentextwide{B} with \screentextwide{5}.
The value of \screentextwide{A} will either be assigned the value \screentextwide{-1} (for {\bf TRUE}), or
the value \screentextwide{0} (for {\bf FALSE}).
You may have noticed that the value of \screentextwide{-1} for {\bf TRUE} is different to
other programming languages, such as {\bf C}, where the value of {\bf 1}
is used for {\bf TRUE} instead.

The \screentextwide{+} symbol can be used a positive sign for numerical expressions,
as an addition operator, or for string concatenation. The number and type of operands
determines the operation.

The \screentextwide{-} symbol can be used a negative sign for numerical expressions,
or as a subtraction operator. The number and type of operands
determines the operation.

The operators \screentextwide{NOT, AND, OR} and \screentextwide{XOR} can be used both as
logical operators, or as boolean operators.

\begin{itemize}
  \item {\bf Logical Operator Example}: \screentextwide{IF A>B AND A<0}
  \item {\bf Boolean Operator Example}: \screentextwide{A = B AND \$7F}
\end{itemize}
...
In the boolean operator example above, the \screentextwide{AND} operator converts both operands
to a 16-bit integer value, and performs a bitwise {\bf AND} for all 16 bits.
This example will take the value of \screentextwide{B}, set the upper 9 bits to zero,
and store the result in \screentextwide{A}.

The result of logical operations can be used in numerical expressions as well,
for example,
\screentextwide{A = A - (B > 7)}
will increment \screentextwide{A} by 1 if the result of \screentextwide{(B > 7)} is {\bf TRUE} (-1).

The operators have precedences, which are listed in the tables below.
In the statement {\ttfamilyA * A - B * B}
...
{\ttfamily A * (A - B) * B)} will execute the subtraction first.
...
\hline
 =   & assignment & {\ttfamily A = 42, A\$ ="HELLO", A = B < 42}\\
\hline
\end{tabular}
\end{center}
...
\hline
plus  & {\ttfamily +} & Positive sign & {\ttfamily A = +42} \\
minus & {\ttfamily -} & Negative sign & {\ttfamily B = -42} \\
\hline
\end{tabular}
\end{center}
...
{\bf Name} & {\bf Symbol} & {\bf Description} & {\bf Example}\\
\hline
Plus           & {\ttfamily +}   & Addition       & {\ttfamily A = B + 42}\\
Minus          & {\ttfamily -}   & Subtraction    & {\ttfamily B = A - 42}\\
Asterisk       & {\ttfamily *}   & Multiplication & {\ttfamily C = A * B}\\
Slash          & {\ttfamily /}   & Division       & {\ttfamily D = B / 13}\\
Up Arrow       & {\ttfamily $\uparrow$} & exponentiation & {\ttfamily E = 2 $\uparrow$ 10}\\
\hline
\end{tabular}
\end{center}
...
\hline
{\ttfamily >}  & Greater than             & {\ttfamily A > 42}  \\
{\ttfamily >=} & Greater than or equal to & {\ttfamily B >= 42} \\
{\ttfamily <}  & Less than                & {\ttfamily A < 42}  \\
{\ttfamily <=} & Less than or equal to    & {\ttfamily B <= 42} \\
{\ttfamily =}  & Equal                    & {\ttfamily A = 42}  \\
{\ttfamily <>} & Not Equal                & {\ttfamily B <> 42} \\
\hline
\end{tabular}
\end{center}
...
\hline
{\ttfamily AND}  & And              & {\ttfamily A > 42 AND A < 84} \\
{\ttfamily OR}   & Or               & {\ttfamily A > 42 OR  A = 0} \\
{\ttfamily XOR}  & Exclusive Or     & {\ttfamily A > 42 XOR B > 42} \\
{\ttfamily NOT}  & Negation         & {\ttfamily C = NOT A > B} \\
\hline
\end{tabular}
\end{center}
...
{\bf Keyword} & {\bf Description} & {\bf Example}\\
\hline
{\ttfamily AND}  & And              & {\ttfamily A = B AND \$FF} \\
{\ttfamily OR}   & Or               & {\ttfamily A = B OR \$80} \\
{\ttfamily XOR}  & Exclusive Or     & {\ttfamily A = B XOR 1} \\
{\ttfamily NOT}  & Negation         & {\ttfamily A = NOT 22} \\
\hline
\end{tabular}
\end{center}
...
\hline
Plus & {\ttfamily +} & Concatenates strings & string & {\ttfamily A\$ = B\$ + ".PRG"} \\
\hline
\end{tabular}
\end{center}
...
\hline
High & {\ttfamily $\uparrow$} \\
& {\ttfamily + -} signs \\ %is this needed here with the +- 2 lines below?
& {\ttfamily * /} \\
& {\ttfamily + -} \\
& {\ttfamily < <= > >= = <>} \\
& {\ttfamily NOT} \\
...
You can also choose no option by leaving it out. \\
\hline
\{ , \} &
...
\item [Remarks:] Any un-shifted character that is not part of the string can be
   ...
   For example, {\ttfamily CHANGE "LOOP" TO "OOPS"} will not find
   the BASIC keyword {\ttfamily LOOP}, because the
   keyword is stored as a token and not as text.
   However {\ttfamily CHANGE /LOOP/ TO /OOPS/} will
...
\item [Usage:]  Displays text on a graphic screen.
               ...
               {\setlength{\tabcolsep}{1mm}
                \begin{tabular}{|l|l|l|}
                  \hline
                  \screentextwide{CHR\$(6)}       &  CTRL+F         &  Flip Character \\
                  \screentextwide{CHR\$(18)}      &  RVSON          &  Reverse  \\
                  \screentextwide{CHR\$(146)}     &  RVSOFF         &  Reverse Off \\
                  \screentextwide{CHR\$(21)}      &  CTRL+U         &  Underline\\
                  \screentextwide{CHR\$(25)+"-"}  &  CTRL+Y + "-"   &  Rotate Left\\
                  \screentextwide{CHR\$(25)+"+"}  &  CTRL+Y + "+"   &  Rotate Right\\
                  \screentextwide{CHR\$(26)}      &  CTRL+Z         &  Mirror\\
                  \hline
                  \end{tabular}
                }
                \end{center}
...
\item [Usage:] Enables or disables
               ...
               not intended.
\item [Example:] \screentextwide{COLOR ON} - with colour/attribute handling \\
                 \screentextwide{COLOR OFF} - no colour/attribute handling
...
\item [Remarks:]
   ...
   about the number or position of different valued bytes.
    {\bf DVERIFY} exits either with the message {\ttfamily OK}
    or with {\ttfamily VERIFY ERROR}.
...
\item [Usage:] Switches the
               ...
               mode, a security prompt {\ttfamily ARE YOU SURE?}
               is displayed, which must be responded with {\ttfamily Y} to
               continue. {\ttfamily SYS58552} can be used to switch back
               to C65-mode.
...
\item [Remarks:]
   ...
   disk drive. This means you don't need to add {\ttfamily ,8} to
...
\setlength{\tabcolsep}{1mm}
    ...
    \hline
        \screentextwide{NOT 0}  &  \screentextwide{1} \\
        \screentextwide{NOT 1}  &  \screentextwide{0} \\
    \hline
\end{tabular}
\end{center}
...
\hline
{\ttfamily 0}    & Keyboard \\
{\ttfamily 1}    & System Default \\
{\ttfamily 2}    & RS232 Serial Connection \\
{\ttfamily 3}    & Screen \\
{\ttfamily 4-7}  & IEC Printer and Plotter \\
{\ttfamily 8-31} & IEC Disk Drives \\
\hline
\end{tabular}
}
\end{center}
...
\setlength{\tabcolsep}{1mm}
    \begin{tabular}{|l|l|l|}
        ...
        {\bf Expression} & {\bf Result}  \\
        \hline
            \screentextwide{0 OR 0}  &  \screentextwide{0} \\
            \screentextwide{0 OR 1}  &  \screentextwide{1} \\
            \screentextwide{1 OR 0}  &  \screentextwide{1} \\
            \screentextwide{1 OR 1}  &  \screentextwide{1} \\
        \hline
    \end{tabular}
\end{center}
...
\hline
\screentextwide{\#} & Sharp \\
\screentextwide{\$} & Flat \\
\screentextwide{.}  & Dotted \\
\screentextwide{H}  & Half Note \\
\screentextwide{I}  & Eighth Note \\
\screentextwide{Q}  & Quarter Note \\
\screentextwide{R}  & Pause (rest) \\
\screentextwide{S}  & Sixteenth Note \\
\screentextwide{W}  & Whole Note \\
\hline
\end{tabular}
}
\end{center}
...
\hline
\screentextwide{O} & Octave              & 0 - 6 \\
\screentextwide{T} & Instrument Envelope & 0 - 9 \\
\screentextwide{U} & Volume              & 0 - 9 \\
\screentextwide{X} & Filter              & 0 - 1 \\
\screentextwide{M} & Modulation          & 0 - 9 \\
\screentextwide{P} & Portamento          & 0 - 9 \\
\screentextwide{L} & Loop                & N/A   \\
\hline
\end{tabular}
}
\end{center}
...
\end{tcolorbox}
    ...
    {\ttfamily DIR "TA*"}, and then view the contents of the file
    by typing {\ttfamily TYPE "TABLE"}.
\end{description}
...
\item [Usage:]  Parses the {\bf format} string and evaluates the argument.
                ...
                CBM style, providing a pattern such as \screentextwide{\#\#\#.\#\#}
                or in C style using a <width.precision> specifier, such as \screentextwide{\%3D \%7.2F \%4X }.

                {\bf argument} the number to be formatted. If the argument does not fit into the format
                e.g. trying to print a 4 digit variable into \screentextwide{\#\#\#}
                ...
                codes are executed.
                The number of \screentextwide{\#} characters sets the width of the output.
...
\hline
\screentextwide{0}  & Open   (\screentextwide{1}), Closed (\screentextwide{0}), or Invalid (>1)  \\
\screentextwide{1}  & Width  (\screentextwide{0}=320, \screentextwide{1}=640)  \\
\screentextwide{2}  & Height (\screentextwide{0}=200, \screentextwide{1}=400)  \\
\screentextwide{3}  & Depth (1-8 Bitplanes)  \\
\screentextwide{4}  & Bitplanes Used  (Bitmask)  \\
\screentextwide{5}  & Bank 4 Blocks Used (Bitmask)  \\
\screentextwide{6}  & Bank 5 Blocks Used (Bitmask)  \\
\screentextwide{7}  & Drawscreen \# (0-3)  \\
\screentextwide{8}  & Viewscreen \# (0-3)  \\
\screentextwide{9}  & Drawmodes  (Bitmask)  \\
\screentextwide{10} & pattern type  (bitmask)  \\
\hline
\end{tabular}
}
\end{center}
...
{\bf Value}  & {\bf Status} \\
\hline
\screentextwide{0}   & No Button \\
\screentextwide{1}   & Right Button \\
\screentextwide{128} & Left Button \\
\screentextwide{129} & Both Buttons \\
\hline
\end{tabular}
}
\end{center}
...
\item [Remarks:]
   ...
   about the number or position of different valued bytes.
    {\bf VERIFY} exits with either the message {\ttfamily OK}
   or with {\ttfamily VERIFY ERROR}.
