...
\multicolumn{2}{|l|}{type \& item size} & appended character & range    & example  \\
\hline
...
\item [Remarks:] The BOX command with four coordinate pairs can be used
...
\item [Usage:]
   "Binary SAVE" saves a memory range to a file of type PRG.
   ...
   which is stored in the first two bytes of the PRG file.
...
\item [Usage:]
   ...
   that is stored in the first two bytes of the PRG file that will be used.
...
Below is a screenshot showing how the directory looks with the {\bf w}ide parameter:
...
\item [Usage:]  Performs a find/replace of the current BASIC program.
   Searches the line range (if specified), otherwise the whole BASIC program is searched.
...
\item [Usage:]
   ...
   the TO token are equal, the COPY command is executed on the disk drive
   ...
   the COPY command is executed in MEGA65 BASIC by reading the source
...
\item [Usage:] Used to define constants
               ...
               in a program. Numbers and strings are allowed, but no expressions.
...
\item [Remarks:]
   ...
   files may be incomplete (commonly called splat files) and no longer usable.
