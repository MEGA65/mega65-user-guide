...
\item [Remarks:]  {\bf BLOAD} cannot cross bank boundaries.
                  ...
                  It is possible to coerce {\bf BLOAD} to load a file of type {\bf SEQ} by using a filename suffix of: \screentext{",S"}. Be sure to also enable raw mode ({\bf ,R}) to avoid treating the first two bytes as a {\bf PRG} load address.
...
\item [Usage:]    Saves a memory range to a file of type {\bf PRG}. (``Binary save.'')
                  ...
                  For compatibility reasons with older BASIC versions, {\bf BSAVE} accepts the syntax with 16-bit addresses at {\bf P} and a bank number at {\bf B} as well. The attic RAM is out of range for this compatibility mode. This mode cannot cross bank boundaries, so the start and end address must be in the same bank.
...
\item [Remarks:]  The length of the file is {\bf end - start + 2}.
                  ...
                  To save a file of type {\bf SEQ}, use a filename suffix of \screentext{",S"}. It is typical to omit the 16-bit load address for files of the type. To do so, provide the {\bf ,R} argument to activate ``raw mode.''
...
\item [Usage:]    Changes the current working directory.
                  ...
                  Depending on the {\bf unit} type, {\bf CHDIR} is applied to different filesystems.
...
\item [Returns:]  The cosine of an angle.
                  ...
                  The {\bf COS} function expects an argument in units of radians.
...
\item [Returns:]  Statistics about how memory is used, or other system properties.
                  ...
                  {\bf FRE(4)} and {\bf FRE(5)} return memory usage of banks 4 and 5, respectively, as an 8-bit bitfield. Each bit represents 8KB of memory in the bank. For example, bit 0 of {\bf FRE(4)} represents the memory region 4.0000 -- 4.1FFF. If a bit is set, then it has been reserved, either by {\bf MEM} or by the system. See {\bf MEM}.
...
\item [Usage:]    Gets the next character, or byte value of the next character, from the keyboard queue. If the queue is empty, the program will wait until a key has been pressed.
                  ...
                  After a key has been pressed, the {\bf variable} will be set and program execution will continue. When used with a string variable, a one character string is created and assigned. Otherwise if the variable is of type numeric, the byte value is assigned.
...
\newpage
\subsection{HASBIT}
\index{BASIC 65 Functions!HASBIT}
\begin{description}[leftmargin=2cm,style=nextline]
\item [Token:]    \$CE \$13
...
\item [Usage:]    Reserves memory in banks 4 or 5, such that the bitmap graphics system will not use it.
                  ...
                  {\bf mask4} and {\bf mask5} are byte values, interpreted as a bitfield. Each bit set to 1 represents an 8KB segment of memory reserved for program use. The first argument describes bank 4, and the second argument describes bank 5.
                  \texttt{
                  \setlength{\tabcolsep}{1mm}
                  \begin{center}
                  \begin{tabular}{|l|l|}
...
\item [Remarks:]  After reserving memory with {\bf MEM}, the graphics library will not use the reserved areas, so it can be used for other purposes. Access to bank 4 and 5 is possible with the commands {\bf PEEK, WPEEK, POKE, WPOKE} and {\bf EDMA}.
                  ...
                  If a graphics screen cannot be opened because the remaining memory is not sufficient, the {\bf SCREEN} command throws an Out of Memory error.

                  Some direct mode commands such as {\bf RENUMBER} use memory in banks 4 and 5 and do not honour {\bf MEM} reservations. {\bf MEM} reservations are only guaranteed during program execution.

                  When 80 $\times$ 50 text mode is enabled, segment 0 of bank 4 is reserved automatically and used for screen data. It always uses segment 0, even if it was previously reserved with {\bf MEM} or a graphic screen. If your program uses 80 $\times$ 50 text mode, be sure not to use segment 0 of bank 4 for other purposes.

                  To deallocate memory reserved by {\bf MEM}, call {\bf MEM} again with the bits set to 0. To deallocate memory reserved by {\bf SCREEN}, close the screen. See {\bf SCREEN CLOSE}. To deallocate memory reserved by 80 $\times$ 50 text mode, switch to one of the other text modes.

                  {\bf FRE(4)} and {\bf FRE(5)} return bitfields that represent the memory reserved in banks 4 and 5, respectively, by {\bf MEM}, {\bf SCREEN}, and 80 $\times$ 50 text mode combined. See {\bf FRE()}.
...
\newpage
\subsection{RDISK}
\index{BASIC 65 Functions!RDISK}
\begin{description}[leftmargin=2cm,style=nextline]
\item [Token:]    \$CE \$15
...
\newpage
\subsection{RSPRSYS}
\index{BASIC 65 Functions!RSPRSYS}
\begin{description}[leftmargin=2cm,style=nextline]
\item [Token:]    \$CE \$17

\item [Format:]   {\bf RSPRSYS(}n{\bf)}

\item [Returns:]  A sprite system parameter.

                  {\bf n} parameter to return (0):

                  \begin{itemize}
                     \item 0: High resolution sprite mode. 1=enabled, 0=disabled.
                  \end{itemize}

\item [Remarks:]  Refer to the {\bf SPRITE SYS} command for more information.

\item [Example:]  Using {\bf RSPRSYS}
...
\item [Remarks:]  Graphics screens are allocated in banks 4 and 5 of memory. If a BASIC program also uses memory in bank 4 or 5, it can reserve regions of this memory to prevent {\bf SCREEN} from allocating it. This is done with the {\bf MEM} command. See {\bf MEM}.
...
\item [Format:]   {\bf SET DEF} unit \\
                  ...
                  {\bf SET FAST MOUSE} <{\bf ON} | {\bf OFF}> \\
...
\item [Usage:]    {\bf SET DEF} redefines the default unit for disk access, which is initialised to 8 by the DOS. Commands that do not explicitly specify a unit will use this default unit.
                  ...
                  {\bf SET FAST MOUSE} enables or disables doubling the speed of the BASIC {\bf MOUSE}.
...
SET FAST MOUSE ON  : REM ENABLE MOUSE SPEED DOUBLING
...
\item [Returns:]  The sine of an angle.
                  ...
                  The {\bf SIN} function expects an argument in units of radians.
...
\item [Usage:]    {\bf SPRITE CLR} clears all sprite data and sets all pointers and attributes to their default values.
                  ...
                  The {\bf SPRITE} form switches a sprite on or off and sets its attributes:
                  ...
                  {\bf SPRITE SYS} sets properties of the sprite system as a whole. It accepts one named argument: {\bf R hires-mode} enables (1) or disables (0) high resolution sprite mode. When enabled, all sprites use high resolution pixels in both vertical and horizontal dimensions, appearing half the size of low resolution mode. High resolution mode also uses a different coordinate plane for sprites.

\item [Remarks:]  {\bf SPRCOLOR} must be used to set additional colours for multi-colour sprites (mode = 1).

                  {\bf RSPRITE()} returns properties of individual sprites set with the {\bf SPRITE} command.

                  {\bf RSPRSYS()} returns properties of the sprite system set with the {\bf SPRITE SYS} command.
...
\item [Returns:]  The tangent of an angle.
                  ...
                  The {\bf TAN} function expects an argument in units of radians.
