...
\item [Remarks:]
               ...
               to BASIC 2, which raised an error for \screentext{ASC("")}.
...
\begin{description}[leftmargin=2cm,style=nextline]
%\item [Token:] \$??
\item [Format:] {\bf CURSOR} column{\bf,} row{\bf}
...
\item [Usage:]  {\bf PALETTE} can be used to change an
                ...
                {\bf colour} index to palette entry (0 -- 255). PALETTE can define colors beyond the default system palette entries 0 -- 31.
...
\item [Returns:]  Writes one or more bytes into memory
                ...
                \screentext{POKE AD,V} because {\bf POKE} uses the low byte anyway.
...
\item [Remarks:] Seeded random number sequences produce the same
                 ...
                 and other factors during boot, so \screentext{RND(1)} is
                 ...
                 \screentext{RND(1)} become predictable. Use
                 \screentext{RND(0)} to re-seed with an unpredictable value.

                 Each call to \screentext{RND(0)} generates a new seed based
                 on the system clock and other factors. Calling
                 \screentext{RND(0)} repeatedly tends to produce a better
