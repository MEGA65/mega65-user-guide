...
Here, {\bf KEY} is a {\em keyword}.\index{Keywords} Keywords are special words
...
The following examples of the {\bf CIRCLE} command are both valid. They have
...
Expressions can be a number value such as \screentext{23.7}, a string value such
as \screentext{"HELLO"}, or a more complex calculation that combines values,
functions, and operators to describe a number or string value:
\screentext{"LIST"+CHR\$(13)}
...
\screentext{?TYPE MISMATCH ERROR}, to say that the type of expression you gave
doesn't match what it expected. For example, the following command results in a
\screentext{?TYPE MISMATCH ERROR}, because \screentext{"POTATO"} is a string
...
in both immediate mode and in a program.
...
For example, the variable \screentext{AA} can store a number value. The
variable \screentext{AA\$} can store a string value. Commodore BASIC
considers these to be separate variables, even though the names both
begin with \screentext{AA}.
...
the first two letters of the name. \screentext{SPEED} and \screentext{SP}
...
example, \screentext{ENFORCEMENT} is not a valid variable name, because
{\bf FOR} is a keyword. It is common to use short variable names to avoid
these hazards.

Once assigned a value, a variable can be used within an expression with other
...
or the empty string (\screentext{""}) for string type variables.

The variable types and their values are as follows.
...
A variable whose name is a single letter followed by the type symbol (or no
...
variable}. Commodore BASIC also supports a type of variable that can store
multiple values, called an {\em array}.
...
integer index. For example, \screentext{AA(7)} refers to the element of the
array named \screentext{AA()} with index 7. Indexes are ``zero-based:'' the
...
name of the variable, similar to scalar variables. \screentext{AA()} is an
array of real numbers, \screentext{AA\$()} is an array of strings.

Array variable names are considered separate from scalar variable names. The
scalar variable \screentext{AA} has no relationship to the array variable
\screentext{AA()}.

Array variables must be declared before use with their maximum size. BASIC 65
allocates the maximum amount of memory needed for the array. The {\bf DIM}
command declares (``dimensions'') an array.

An array can have multiple dimensions, each with its own index separated by a
...
Screen codes are not the same as PETSCII codes. Refer to a Commodore reference
for a list of screen codes.

\textbf{C@\&(}column{\bf ,} row{\bf )} is the colour code of the character.
This is an entry number of the system palette. See the colour table on page
\pageref{colourtable}. Upper bits also set text attributes, such as blinking.
...
The part of the expression that involves the operator is treated as the result
of the operation: the operator {\em evaluates to} the result.
...
As a relational operator, the {\bf =} symbol behaves as an expression.
...
to the number variable \screentext{A}. It then expects a number expression on
...
as logical operators or as boolean operators. A logical operator join two
...
Conditional expressions evaluating to numbers can be used in some clever
...
This statement will increment the value in the \screentext{A} by 1 if the
value in \screentext{B} is greater than 7. Otherwise it leaves it unchanged.
If the sub-expression \screentext{B > 7} is true, then it evaluates to -1.
\screentext{A - (-1)} is equivalent to \screentext{A + 1}. If the
sub-expression is false, then it evaluates to 0, and \screentext{A - 0} is
equivalent to \screentext{A}.
...
statement \screentext{A * A - B * B}, both multiplications will be performed
first, then the subtraction. As in algebra, you can use parentheses to change
the order of execution. In the expression \screentext{A * (A - B) * B}, the
subtraction is performed first.

The complete set of operators and their order of predecence are summarized in
...
\item [Usage:]  Returns the absolute value of the numeric
                argument {\bf x}.
...
\item [Usage:] The PETSCII code of the first character of
...
\item [Usage:] The arc tangent of the argument.
...
\item [Usage:] Sets the background colour
               ...
               All colours within this range are
...
\item [Usage:]
   ...
   performs a \screentext{JSR \$0400} afterwards (Jump To Subroutine).
...
\item [Usage:] A bitfield of sprites currently colliding with other
...
\item [Remarks:]
   ...
   In direct mode {\bf BVERIFY} exits either with the message \screentext{OK}
   or with \screentext{VERIFY ERROR}. In program mode, a VERIFY ERROR
...
\item [Usage:]  Prints a file catalog/directory of the specified disk.
   ...
   {\bf filepattern} is either a quoted string, for example: \screentext{"DA*"} or
   a string expression in brackets, e.g. \screentext{(DI\$)}
...
\item [Usage:]  Edits the BASIC program
   ...
   listed and the user is prompted for an action:
    ...
    \end{itemize}
\item [Remarks:] Any punctuation character that is not part of the string can be
   used instead of {\bf /}.
   ...
   find and replace it (possibly causing \screentext{SYNTAX ERROR}s).
...
\item [Usage:] Changes the appearance of a character.
      ...
      (@:0, A:1, B:2, ...).
      % TODO: link to screen code reference when we have one
...
\item [Usage:] A string containing one character of the given PETSCII
               value.
...
\newpage
\subsection{CLRBIT}
\index{BASIC 65 Functions!CLRBIT}
\begin{description}[leftmargin=2cm,style=nextline]
\item [Token:] \$9C \$FE \$4E
...
\item [Usage:] Sets the foreground text colour for subsequent {\bf PRINT} commands.
               ...
               Refer to the table under {\bf BACKGROUND} on page \pageref{colourtable} for
               the palette entry numebrs and their corresponding colours in the default system palette.
...
\item [Usage:]
   ...
   {\bf appendfile} is either a quoted string, for example: \screentext{"DATA"} or
   a string expression in brackets, for example: \screentext{(FI\$)}

   {\bf targetfile} is either a quoted string, for example: \screentext{"SAFE"} or
   a string expression in brackets, for example: \screentext{(FS\$)}
...
\item [Usage:]
   ...
   {\bf source} is either a quoted string, e.g. \screentext{"DATA"} or
   a string expression in brackets, e.g. \screentext{(FI\$)}.

   {\bf target} is either a quoted string, e.g. \screentext{"BACKUP"} or
   a string expression in brackets, e.g. \screentext{(FS\$)}
...
\item [Usage:] The cosine of an angle.
...
\begin{description}[leftmargin=2cm,style=nextline]
%\item [Token:] \$??
\item [Format:] {\bf CURSOR} \{column{\bf,} row{\bf,} style\}
...
\item [Usage:] The decimal value of a hexadecimal string.

               The argument range is ``0'' to ``FFFFFFFF''. {\bf DEC()}
               ignores everything after the first non-hex digit or the eighth
               character.

\item [Remarks:] Allowed digits in uppercase/graphics mode are 0 -- 9 and A -- F (\screentext{0123456789ABCDEF})
                 and in lowercase/uppercase mode are 0 -- 9 and a -- f (\screentext{0123456789abcdef}).
...
\item [Usage:] The first form deletes
   ...
   {\bf filename} is either a quoted string, for example: \screentext{"SAFE""} or
   a string expression in brackets, for example: \screentext{(FS\$)}
...
\item [Remarks:]
   ...
   The shortcut key \megakey{@} can only be used in direct mode.
...
This listing can be scrolled like a BASIC program with the keys \megakey{F9}
or \megakey{F11}, edited, listed, saved or printed.
...
\item [Usage:]  The status of the last disk operation
                ...
                DS\$ is set to \screentext{00,OK,00,00} if there was no error, otherwise
...
\item [Usage:] The string description of a given BASIC error number.
...
\item [Remarks:] Although it's possible to call {\bf FAST}
                 ...
                 {\bf FAST} has no effect if \screentext{POKE 0,65}
...
\item [Remarks:] Any punctuation character that is not part of
...
\item [Usage:]
   ...
   {\bf diskname} is either a quoted string, e.g. \screentext{"DATA"} or
   a string expression in brackets, e.g. \screentext{(DN\$)}.
...
\item [Usage:] The number of free bytes for banks 0 or 1,
...
\screentext{XY()} is \screentext{POINTER(XY(0))}. \screentext{POINTER(XY)}
returns the address of the scalar variable \screentext{XY}.
...
\item [Remarks:]
      ...
      to holding and releasing the \widekey{RESTORE} key.
...
\item [Usage:] Switches the
               ...
               You can switch back to MEGA65 mode with this command: \screentext{SYS58552}
...
\item [Usage:] A four character hexadecimal
...
\item [Remarks:]
   ...
   \screentext{DOPEN\#1,"LISTING",W:CMD 1:LIST:DCLOSE\#1}
...
\item [Remarks:] The user must take care to enter the correct
               ...
               with the prompt \screentext{??}.
               Typing non numeric characters for integer or real
               variables will produce a \screentext{TYPE MISMATCH ERROR}.
...
\item [Remarks:] The type and number of data in a record must
               ...
               variables will produce a \screentext{FILE DATA ERROR}.
...
\item [Usage:] The integer part of a number.
...
\item [Usage:] Manages the function key macros in the BASIC editor.
               ...
               {\bf number} can be any value within this range:
                ...
                \begin{itemize}
                    \item {\bf 1 - 14}: corresponds to keys ranging from \megakey{F1} to \megakey{F14}
                    \item {\bf 15}: corresponds to \specialkey{HELP}
                    \item {\bf 16}: corresponds to \specialkey{SHIFT}\specialkey{RUN STOP}
                \end{itemize}
...
\item [Usage:] The first {\bf n} characters from the
               argument {\bf string}.
...
\item [Usage:] The length of a string.
...
\item [Remarks:] This differs from {\bf INPUT} in how the input is parsed.
                ...
                a single string value. Only the \specialkey{RETURN} key does
...
\item [Usage:] Lists a range of lines from the BASIC program in memory.
               ...
               (\screentext{-}), a starting number and a hyphen, or a hyphen
...
\item [Remarks:]
                ...
                Another way to display a program listing from memory on the screen is to use the keys \megakey{F9} and \megakey{F11}, or
...
or Relative file.
   ...
   {\bf PRG} files. To do this:
    ...
    \end{enumerate}
   After pressing \specialkey{RETURN}, the listed file on the line
   ...
   {\bf filename} is either a quoted string, e.g. \screentext{"PROG"}, or
   a string expression.
...
\item [Usage:] The natural logarithm of a number.
...
\item [Usage:] The decimal logarithm of the argument.
...
\item [Usage:] Reads the state of a light pen peripheral.
...
\item [Usage:]  Reserves memory in banks 4 or 5 such that the
                bitmap graphics system will not use it.

                {\bf mask4} and {\bf mask5} are byte values, that
                are interpreted as mask of 8 bits. Each bit set to 1
                reserves an 8K segment of memory in bank 4 for the first
                argument and in bank 5 for the second argument.
...
\item [Remarks:] After reserving memory with {\bf MEM} the graphics
                 library will not use the reserved areas, so it can
                 be used for other purposes. Access to bank 4 and 5
                 is possible with the commands {\bf PEEK, WPEEK, POKE, WPOKE}
                 and {\bf EDMA}.

                 If a graphics screen cannot be opened, because the
                 remaining memory is not sufficient, the program
                 stops with a \screentext{?OUT OF MEMORY ERROR}.

                 Some direct mode commands like {\bf RENUMBER} use memory in
                 banks 4 and 5 and do not honor {\bf MEM} reservations. Such
                 reservations are only guaranteed during program execution.
...
\item [Usage:] Makes (creates) a subdirectory on a floppy or D81 disk image.
   ...
   The error message \screentext{DISK FULL} is reported if there isn't such a region.
...
\item [Usage:] The remainder of a division operation.
...
\item [Remarks:] The "hot spot" of the mouse pointer is the upper left
                ...
                When the system boots, sprite 0 is initialized to a picture of
                a mouse pointer.
...
\item [Remarks:] The {\bf index} variable after {\bf NEXT} is
               ...
               \screentext{NEXT I:NEXT J:NEXT K} and
               \screentext{NEXT I,J,K} are equivalent.
...
\end{center}
   ...
   {\bf filename} is either a quoted string, e.g. \screentext{"DATA"} or
   a string expression. The syntax is different to {\bf DOPEN\#},
   since the {\bf filename} for {\bf OPEN} includes all
   file attributes, for example: \screentext{"0:DATA,S,W"}.
...
\item [Usage:]  The byte value stored in memory at {\bf address}, as an
...
\item [Usage:]  Bitmap graphics: sets the colour of the graphic pen for the current screen.
                ...
                Refer to the table under {\bf BACKGROUND} on
                page \pageref{colourtable} for the palette entries of the system palette.
...
\end{center}
  ...
  You can omit a string for a given voice, such as to allow a looping pattern
  on that voice to continue playing, using empty arguments:
...
\item [Remarks:] The {\bf PLAY} statement makes use of an interrupt driven
                 ...
                 different from the Commodore 128, which stops program
...
\item [Usage:]  The current address of a variable
...
\item [Usage:]  Writes one or more bytes into memory
                ...
                \screentext{POKE AD,V AND 255} can be written as
                \screentext{POKE AD,V} because {\bf POKE} uses the lowy byte anyway.
...
\item [Usage:]  The cursor column relative to the
...
\item [Usage:]  The position of a paddle peripheral.
...
\item [Usage:]  Prints a series of values formatted using a pattern to the current output
                ...
                CBM style, providing a pattern such as \screentext{\#\#\#.\#\#}
                or in C style using a <width.precision> specifier, such as \screentext{\%3D \%7.2F \%4X }.
...
\item [Usage:]  The current colour index for the
...
\item [Usage:]  Positions the read/write pointer of a relative file.
               ...
               status will give the message \screentext{RECORD NOT PRESENT}, but
...
\item [Usage:] Renames a disk file.
   ...
   {\bf old} is either a quoted string, e.g. \screentext{"DATA"} or
   a string expression in brackets, e.g. \screentext{(FI\$)}.

   {\bf new} is either a quoted string, e.g. \screentext{"BACKUP"} or
   a string expression in brackets, e.g. \screentext{(FS\$)}
...
\item [Usage:]  Bitmap graphics: the status of a given graphic screen parameter.
...
\item [Usage:] The last {\bf n} characters from {\bf string}.
...
\item [Usage:] Generates a new pseudo random number.

               This is called a "pseudo" random number as
               the numbers are not {\em really} random. They are derived
...
\item [Usage:]  The red, green, or blue value of
...
\item [Usage:]  The colour index of pen {\bf n}.
...
\item [Usage:] Tests whether music is playing on the given voice channel.
...
\item [Usage:]  The colour setting of a multi-colour sprite colour.
...
\item [Usage:]  The current CPU clock in MHz.

                {\bf n} numeric dummy argument, which is ignored.

\item [Remarks:] {\bf RSPEED(n)} will not return the correct value if \screentext{POKE 0,65}
...
\item [Usage:]  A sprite's position or speed.
...
\item [Usage:] Runs the BASIC program in memory, or loads and runs a program from disk.
   ...
   {\bf filename} either a quoted string, e.g. \screentext{"PROG"} or
   a string expression in brackets, e.g. \screentext{(PR\$)}.
...
\item [Usage:]  A parameter of the current text window.
...
\item [Remarks:] {\bf SAVE} is obsolete, implemented only for backwards compatibility.
                 ...
                 The shortcut symbol \megakeywhite{$\leftarrow$} is next to \megakey{1}. Can only be used in direct mode.
...
\newpage
\subsection{SETBIT}
\index{BASIC 65 Functions!SETBIT}
\begin{description}[leftmargin=2cm,style=nextline]
...
\item [Usage:] The sign of a numeric expression, as a number.
...
\item [Usage:] The sine of an angle.
...
\item [Usage:] A string of cursor-right PETSCII codes, suitable for printing
               to advance the cursor the given number of columns.
...
\item [Remarks:] Although it's possible to call {\bf SPEED}
                ...
                {\bf SPEED} has no effect if \screentext{POKE 0,65}
                has previously been used to set the CPU to 40MHz.
...
\item [Remarks:] The colours used with {\bf SPRCOLOR} will affect all sprites.
                 ...
                 The final argument to {\bf SPRITE} enables multi-color mode
                 for the sprite.
...
\item [Usage:] The square root of a numeric expression.
...
\item [Usage:] A string of the formatted value of the argument,
...
\item [Usage:] The tangent of an angle.
...
\item [Usage:] The decimal floating point value represented by a string.
...
\item [Remarks:] The clipping region can be reset to full screen by the command\\
                 \screentext{VIEWPORT DEF 0,0,WIDTH,HEIGHT}
...
\item [Usage:]  The 16-bit word value stored in memory at {\bf address} (low byte) and
...
\item [Usage:]  Writes one or more 16-bit words into memory
                or memory mapped I/O, starting at {\bf address}.
