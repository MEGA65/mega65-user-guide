...
\hline
Decimal Integer         &  {\ttfamily 32000} & {\ttfamily -55}      \\
Decimal Fixed Point     &  {\ttfamily 3.14}  & {\ttfamily -7654.321}\\
Decimal Floating Point  &  {\ttfamily 1.5E03}& {\ttfamily 7.7E-02}  \\
Hex                     &  {\ttfamily \$D020}& {\ttfamily \$FF}     \\
String                  &  {\ttfamily "X"}   & {\ttfamily "TEXT"}   \\
\hline
\end{tabular}
\end{center}
...
\hline
Byte                    & {\ttfamily \&} & 0 .. 255          & {\ttfamily BY\& = 23}    \\
Integer                 & {\ttfamily \%} & -32768 .. 32767   & {\ttfamily I\% = 5}      \\
Real                    & none           & -1E37 .. 1E37     & {\ttfamily XY = 1/3}     \\
String                  & {\ttfamily \$} & length = 0 .. 255 & {\ttfamily AB\$ = "TEXT"}\\
\hline
\end{tabular}
\end{center}
...
\hline
Byte     Array &  1     &    {\ttfamily \&}    & 0 .. 255          & {\ttfamily BY\&(5,6) = 23}   \\
Integer  Array &  2     &    {\ttfamily \%}    & -32768 .. 32767   & {\ttfamily I\%(0,10) = 5}    \\
Real     Array &  5     &   none               & -1E37 .. 1E37     & {\ttfamily XY(I\%) = 1/3}    \\
String   Array &  3     &    {\ttfamily \$}    & length = 0 .. 255 & {\ttfamily AB\$(X) = "TEXT"} \\
\hline
\end{tabular}
\end{center}
...
The {\ttfamily =} symbol is used both as an assignment operator, and as a relational
operator for testing equality. For example, in the statement
{\ttfamily A = B = 5}, the first equal sign is the assignment operator,
while the second is a logical operator, comparing the variable {\ttfamily B} with {\ttfamily 5}.
The value of {\ttfamily A} will either be assigned the value {\ttfamily -1} (for {\bf TRUE}), or
the value {\ttfamily 0} (for {\bf FALSE}).
You may have noticed that the value of {\ttfamily -1} for {\bf TRUE} is different to
other programming languages, such as {\bf C}, where the value of {\bf 1}
is used for {\bf TRUE} instead.

The {\ttfamily +} symbol can be used a positive sign for numerical expressions,
as an addition operator, or for string concatenation. The number and type of operands
determines the operation.

The {\ttfamily -} symbol can be used a negative sign for numerical expressions,
or as a subtraction operator. The number and type of operands
determines the operation.

The operators {\ttfamily NOT, AND, OR} and {\ttfamily XOR} can be used both as
logical operators, or as boolean operators.

\begin{itemize}
  \item {\bf Logical Operator Example}: {\ttfamily IF A>B AND A<0}
  \item {\bf Boolean Operator Example}: {\ttfamily A = B AND \$7F}
\end{itemize}
...
In the boolean operator example above, the {\ttfamily AND} operator converts both operands
to a 16-bit integer value, and performs a bitwise {\bf AND} for all 16 bits.
This example will take the value of {\ttfamily B}, set the upper 9 bits to zero,
and store the result in {\ttfamily A}.

The result of logical operations can be used in numerical expressions as well,
for example,
{\ttfamily A = A - (B > 7)}
will increment {\ttfamily A} by 1 if the result of {\ttfamily (B > 7)} is {\bf TRUE} (-1).

The operators have precedences, which are listed in the tables below.
In the statement {\ttfamily A * A - B * B}
...
\item [Usage:]  Displays text on a graphic screen.
               ...
               {\setlength{\tabcolsep}{1mm}
                \begin{tabular}{|l|l|l|}
                  \hline
                  {\ttfamily CHR\$(6)}       &  CTRL+F         &  Flip Character \\
                  {\ttfamily CHR\$(18)}      &  RVSON          &  Reverse  \\
                  {\ttfamily CHR\$(146)}     &  RVSOFF         &  Reverse Off \\
                  {\ttfamily CHR\$(21)}      &  CTRL+U         &  Underline\\
                  {\ttfamily CHR\$(25)+"-"}  &  CTRL+Y + "-"   &  Rotate Left\\
                  {\ttfamily CHR\$(25)+"+"}  &  CTRL+Y + "+"   &  Rotate Right\\
                  {\ttfamily CHR\$(26)}      &  CTRL+Z         &  Mirror\\
                  \hline
                  \end{tabular}
                }
                \end{center}
...
\item [Usage:] Enables or disables
               ...
               not intended.
\item [Example:] {\ttfamily COLOR ON} - with colour/attribute handling \\
                 {\ttfamily COLOR OFF} - no colour/attribute handling
...
\setlength{\tabcolsep}{1mm}
    ...
    \hline
        {\ttfamily NOT 0}  &  {\ttfamily 1} \\
        {\ttfamily NOT 1}  &  {\ttfamily 0} \\
    \hline
\end{tabular}
\end{center}
...
\setlength{\tabcolsep}{1mm}
    \begin{tabular}{|l|l|l|}
        ...
        {\bf Expression} & {\bf Result}  \\
        \hline
            {\ttfamily 0 OR 0}  &  {\ttfamily 0} \\
            {\ttfamily 0 OR 1}  &  {\ttfamily 1} \\
            {\ttfamily 1 OR 0}  &  {\ttfamily 1} \\
            {\ttfamily 1 OR 1}  &  {\ttfamily 1} \\
        \hline
    \end{tabular}
\end{center}
...
{\ttfamily .}  & Dotted \\
{\ttfamily H}  & Half Note \\
{\ttfamily I}  & Eighth Note \\
{\ttfamily Q}  & Quarter Note \\
{\ttfamily R}  & Pause (rest) \\
{\ttfamily S}  & Sixteenth Note \\
{\ttfamily W}  & Whole Note \\
\hline
\end{tabular}
}
\end{center}
...
\hline
{\ttfamily O} & Octave              & 0 - 6 \\
{\ttfamily T} & Instrument Envelope & 0 - 9 \\
{\ttfamily U} & Volume              & 0 - 9 \\
{\ttfamily X} & Filter              & 0 - 1 \\
{\ttfamily M} & Modulation          & 0 - 9 \\
{\ttfamily P} & Portamento          & 0 - 9 \\
{\ttfamily L} & Loop                & N/A   \\
\hline
\end{tabular}
}
...
\hline
{\ttfamily 0}  & Open   ({\ttfamily 1}), Closed ({\ttfamily 0}), or Invalid (>1)  \\
{\ttfamily 1}  & Width  ({\ttfamily 0}=320, {\ttfamily 1}=640)  \\
{\ttfamily 2}  & Height ({\ttfamily 0}=200, {\ttfamily 1}=400)  \\
{\ttfamily 3}  & Depth (1-8 Bitplanes)  \\
{\ttfamily 4}  & Bitplanes Used  (Bitmask)  \\
{\ttfamily 5}  & Bank 4 Blocks Used (Bitmask)  \\
{\ttfamily 6}  & Bank 5 Blocks Used (Bitmask)  \\
{\ttfamily 7}  & Drawscreen \# (0-3)  \\
{\ttfamily 8}  & Viewscreen \# (0-3)  \\
{\ttfamily 9}  & Drawmodes  (Bitmask)  \\
{\ttfamily 10} & pattern type  (bitmask)  \\
\hline
\end{tabular}
}
\end{center}
...
\hline
{\ttfamily 0}   & No Button \\
{\ttfamily 1}   & Right Button \\
{\ttfamily 128} & Left Button \\
{\ttfamily 129} & Both Buttons \\
\hline
\end{tabular}
}
\end{center}
