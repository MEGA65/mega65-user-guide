...
\item [Usage:]  Performs a {\bf find and replace} of the BASIC programi
                that is currently in memory.
   A specified line range limits the seaarch to this range.
   Otherwise the whole BASIC program is searched.
   At each occurrence of the findstring, the line is
...
\item [Remarks:]
   ...
   splat files it is not recommended for disks with boot blocks or random access files.
...
\item [Token:] \$FE \$0D
\item [Format:]
  {\bf DOPEN\# lfn, filename [,L[recordlength]] [,W] [,D drive] [,U unit] }
\item [Usage:]
   ...
   is opened for read/write, as well as random access. The {\bf recordlength}
   is mandatory for creating relative files. Although optional,
   the {\bf recordlength} is used as a safety check for existing relative files.
...
\item [Usage:]
   "Drawing PATtern" sets pattern the
...
\item [Usage:]  {\bf DS} holds the status of the last disk operation.
                ...
                will return 0 if no other drive activity has taken place.
...
\item [Format:] {\bf DS\$} is a reserved system variable.
\item [Usage:]  {\bf DS\$} holds the status of the last disk operation
                in text form in the folloing format:
                Code,Message,Track,Sector.

                {\bf DS\$} is coupled to the numeric variable {\bf DS}.
                It is updated when {\bf DS} is used.
                {\bfDS\$} is set to "00,OK,00,00" if there was no error, otherwise
                it is set to a DOS error message (listed in the manual
                of your disk drive).
...
\item [Usage:]  {\bf EDIT} switches the built-in editor
               either to text mode when using {\bf EDIT ON},
               or BASIC program editor mode with {\bf EDIT OFF}.

               After power up or reset, the editor defaults
               to the BASIC program editor mode.

                The mode of the editor can be identified by looking at the prompt.
                In program mode the prompt is {\bf READY}, whereas in text mode
                the prompt is {\bf OK}.
               ...
               {\bf EDIT ON}, the differences to program mode are:
                ...
                \begin{itemize}
                \item The editor does no tokenising. All text entered after a linenumber remains pure text,
                      BASIC keywords such as {\bf FOR} or {\bf GOTO} are not
                      converted to BASIC tokens, like in program mode.
                \item Line numbers are only used for text organisation,
                        sorting, deleting, listing etc.
                        When text is saved to file with {\bf DSAVE},
                        a sequential file (type {\bf SEQ}) is written, as opposed to a
                        program ({\bf PRG}) file, which is the case when in program mode.
                \item Line numbers are not written to disk.
                \item Text mode only affects lines with a leading number,
                        lines with no linenumber are executed as BASIC commands.
                \item Sequential files, created with the text editor, can be displayed
                        (without loading them) on screen by using the {\bf TYPE <filename>} command.
                \end{itemize}


% TODO newpage isn't creating a new page here
\newpage
...
using up to 28 bits for the addresses of source and target.
