...
\item [Usage:] Enables faster typing of BASIC programs.
  ...
  the \specialkey{RETURN} key, the AUTO function generates a new BASIC line
...
\item [Usage:]
   ...
   {\bf start} address where saving begins from.
...
\item [Usage:]  {\bf FIND} is an editor command that can only be used
                ...
                The \specialkey{NO\\SCROLL} key can be used to pause the output.
...
\item [Remarks:] {\bf SAVE} is obsolete, implemented only for backwards compatibility.
                 ...
                 The shortcut symbol {\bf $\leftarrow$} (next to the \megakey{1} key). Can only be used in direct mode.
...
%TODO there isn't enough spacing here and i dont know how to add some. \\ isn't working
        \underline{{\bf Detailed approach}}:
               ...
               apart from {\bf SCREEN CLOSE}.
                ...
                {\bf screen} screen number 0-3
                {\bf width flag} 0-1 (0:320, 1:640 pixels)
                {\bf height flag} 0-1 (0:200, 1:400 pixels)
                {\bf depth} 1-8 (2 - 256 colours)
...
\item [Remarks:] Pressing the \specialkey{RUN STOP} key interrupts the sleep.
...
\item [Usage:]  Sets multi-colour sprite colours.
                ...
                The colours used with {\bf SPRCOLOR} will affect all sprites.
...
\item [Remarks:] The register values after a {\bf SYS} call are stored
                 ...
                 RAM with no I/O or kernel mapped in.\\
                 \\
                 Using {\bf SYS} properly (i.e. without corrupting BASIC RAM, and having
                 access to kernel routines and I/O) requires
...
\item [Remarks:] {\bf TI} is a reserved system variable. The value in {\bf TI}
                 is the number of seconds (with 6 decimal places) since it was last cleared or started.
...
\item [Remarks:]
   {\bf VIEWPORT} must be followed either by {\bf CLR} or {\bf DEF}, and four integer parameters.
...
\item [Remarks:] {\bf WAIT} is typically used to examine hardware
               ...
               mouse event, keyboard press or a speficic raster line
