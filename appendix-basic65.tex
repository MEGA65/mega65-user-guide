...
\item [Usage:]   Bitmap graphics: draws a circle. This is a special case of {\bf ELLIPSE}, using the same value for horizontal and vertical radius.
                 ...
                 {\bf xc} x coordinate of the centre in pixels.

                 {\bf yc} y coordinate of the centre in pixels.
...
\item [Usage:]    Copies a file to another file, or one or more files from one disk to another.
                  ...
                  {\bf source} is either a quoted string, e.g. \screentext{"DATA"} or a string expression in brackets, e.g. \screentext{(FI\$)}.

                  {\bf target} is either a quoted string, e.g. \screentext{"BACKUP"} or a string expression in brackets, e.g. \screentext{(FS\$)}.
...
\item [Usage:]    Bitmap graphics: draws an ellipse.
                  ...
                  {\bf xr} x radius of the ellipse in pixels.

                  {\bf yr} y radius of the ellipse in pixels.
...
\item [Usage:]    Switches the MEGA65 to C64-compatible mode.
                  ...
                  If you're in direct mode, a security prompt \screentext{"ARE YOU SURE?"} is displayed, which must be responded with 'Y' to continue.
...
\item [Usage:]    Performs {\bf GOSUB} or {\bf GOTO} to a line number selected by a number expression.
                  ...
                  {\bf expression} positive numeric value. Real values are converted to integer (losing precision). Logical operands are converted to a 16-bit integers, using \$FFFF (decimal -1) for TRUE, and \$0000 (decimal 0) for FALSE.
