...
The BANK and FLAGS field for the source and destination addresses allow selection of addresses
within a 1MB address space. To access memory beyond the first 1MB, it is necessary to use an
Enhanced DMA Job with the appropriate option bytes to select the source and/or destination MB
of memory. The BANK and FLAGS field has the following structure:
...
\begin{asmcode}
  ; DMA list exists in BANK 0
  LDA #$00
  STA $D702
  ; Set MSB of DMA list address
  LDA #>dmalist
  STA $D701
  ; Set LSB of DMA list address, and execute DMA
  LDA #<dmalist
  STA $D700
  RTS
...
basicheader:
  ...
  ;; Actual code beginning at $080d = 2061
main:
  SEI
...
raster_sync:       ; before beginning loop for horizontal
  CMP $D012        ; alignment
  BEQ raster_sync

  ;; The following loop takes exactly one raster line
  ;; at 40.5MHz in PAL
loop:
  JSR triggerdma
  JMP loop
...
\begin{tabular}{|>{\centering\arraybackslash}p{1.5cm}|p{10cm}|}
  ...
  \$0D & Write raw flux to floppy drive \\
  \$0E & Read raw flux to floppy drive \\
  \$0F & Read raw flux to floppy drive \\
