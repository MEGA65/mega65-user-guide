...
{
  \newcommand{\availablefrom}[1]{Available since Hypervisor ##1}
  \newcommand{\errordesc}[3]{\index{Hypervisor Error Codes!\$##1}\textbf{\$##1 ##2} ##3\par}
...
The Hypervisor is a software component of the Mega65. The Hypervisor:
\begin{itemize}
  ...
  \item Calls up various utilties like the freezer and the Matrix Mode
        Debugger.
  \item Provides services specific to the Mega65 that you can use in your
        programmes.
\end{itemize}

If you know about hypervisors and virtual machines, the Hypervisor is a very
limited hypervisor. Don't expect to be able to run multiple virtual machines
concurrently with full isolation. The Hypervisor runs environments that are
more akin to the task and processes of a modern operating system. The
Hypervisor has a C64-like, a C65-like and a Mega65-native operating mode for
...
\begin{itemize}
  \item \textbf{The C64-like operating mode} runs C64 programmes and Mega65
        ...
        \megasymbolkey pressed or use the \textbf{GO64} command, the Hypervisor
        starts a process in the C64-like operating mode.
  \item \textbf{The C65-like operating mode} is the Mega65's normal operating
        ...
        programmes and 45GS02 machine-code programmes.
...
The Hypervisor provides very limited virtualisation of the Mega65's hardware.
It can virtualise the floppy controller. There are plans to virtualise the
serial bus so the Mega65 can use disk images for units like the 1541.

There are some parts of the hardware that only the Hypervisor can access. It is
the only component that can directly access the internal and external SD cards.
You need to use the Hypervisor's services if you want to access the files and
directories on the SD cards from within your programmes.

\subsection{Terminology}

When you start to learn about the Hypervisor, there can be some terminology that
might be confusing if you already know about other parts of the Mega65.

The Hypervisor has the nick-name HYPPO. In this appendix, we call it the
Hypervisor. On the SD card there is likely to be a file called HICKUP.M65. This
file updates the Hypervisor to new versions without having to install an
upgraded core.

There are 3 distinct disk operating systems in the Mega65.

\begin{itemize}
  \item Inside the Hypervisor is HYPPO DOS, or HDOS for short. HDOS is for
        accessing the FAT32 file system on the SD cards. HDOS does not know
        anything about Commodore file systems. It can attach an image of a
        Commodore file system, but it does not understand what is inside
        the image.
  \item Inside the Kernal is CBDOS. CBDOS is for accessing 1581-like file
        ...
        the sectors of a physical disk. It uses the Hypervisor to access the
        sectors of a disk image. CBDOS does not know anything about SD cards
        and FAT32 file system on them.
...
\begin{itemize}
  \item The drives in the Hypervisor are the partitions of the internal and
        external SD cards. When the Mega65 boots, the Hypervisor assigns
        numbers to the partitions it can read.
  \item The drives in CBDOS are the physical disk drives attached to the 45IO27
        multi-function I/O controller or the disk images attached via the
        Hypervisor. The internal disk drive attaches to the 45IO27. The CBDOS
        drives are normally seen as units 8 and 9.
  \item The drives in an external disk unit attached to the serial bus are the
        disk drives inside that unit.
\end{itemize}

\subsection{Versions}

This chapter describes the services available in Hypervisor 1.2.

New Hypervisor services may become available and existing Hypervisor services
may change or be deprecated. A robust programme will use the hdos\_getversion
service to check whether it is compatible with the Hypervisor in the MEGA65
it's running on.
...
Which hypervisor service starts depends on what value from the A register you
...
When the hypervisor service finishes, the CPU will switch back to your programme.
Except for the registers a service uses to return values, the registers are
otherwise preserved.

\textbf{Important} The CPU may or may not execute the next byte in your programme
...
\item [Precondition:]
  The previous service uses cleared the C flag.
\item [Outputs:]
...
\item [Service:]
  Sets up the area the Hypervisor uses to transfer data to and from your
  programme.
\item [Inputs:]
...
\item [Outputs:]
  ...
  hdos\_closedir. See the remarks.}
\item [Postconditions:]
...
\item [Remarks:]
  ...
  If the Hypervisor doesn't find any matching files, it will fail with a file
  not found error. In this case the Hypervisor will have already closed the
...
\item [History:]
  \availablefrom{1.2}
\item [Remarks:]
  If the Hypervisor doesn't find a subsequent matching file, it will fail with a
...
\item [Postconditions:]
  No additional file descriptors are open.
\item [Errors:]
  \errordesc{84}{too many open files}{hdos\_loadfile uses one file descriptor
  internally, but all the file descriptors are in use. Close some or all of the
  file descriptors using hdos\_closedir, hdos\_closefile or hdos\_closeall.}
...
\item [Errors:]
  ...
  current Hypervisor filename in the current working dirctory of the current
  drive.}
\notimplemented
\end{hyppotrap}
...
\item [Outputs:]
  ...
  responsible for closing this file descriptor using hdos\_closedir.}
\item [Postconditions:]
...
\item [Remarks:]
  ...
  destination area, the Hypervisor skips the entry entrirely.
...
a floppy drive in all cases. If you intend for your programme to work with both
...
% d81write_en
%
% Jimbo - hdos_d81attach* already write-enables the images.
