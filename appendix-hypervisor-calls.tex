...
chapter describes the services available in Hypervisor 1.2.
...
service to check whether it is compatible with the Hypervisor in the
MEGA65 it's running on.
...
mindful of when the CPU interpets the NOP as an prefix for the following
...
in the A register. This is a table of error codes in the description for
hdos\_geterrorocde.
...
\item [Error codes:] This is possibly not an exhaustive list.
{
  ...
  \begin{longtable}{|c|r|l|p{8cm}|}
    ...
    \$05 & 5 & not two FATs &
    The partition that does not have exactly two copies of the FAT structure.
    \\\hline
...
\item [Postconditions:]
  No additional file descriptors are open.
\item [Errors:]
  \errordesc{84}{too many open files}{loadfilec uses one file descriptor
...
\item [Postconditions:]
  No additional file descriptors are open.
\item [Errors:]
  \errordesc{84}{too many open files}{loadfile\_attic uses one file descriptor
  internally, but all the file descriptors are in use. Close some or all of the
  file descriptors using hdos\_closedir, hdos\_closefile or hdos\_closeall.}
  \errordesc{88}{file not found}{The file was not found in the current directory
  of the current drive.}
