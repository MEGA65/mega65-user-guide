...
For example, \screentext{LDA \$12,X} would read the value stored in location (\$12 + X) in the Base-Page,
...
The argument is encoded as a single byte that immediately follows the instruction opcode.
For example, \screentext{DEQ \$12,X} would increment the 32-bit word stored at (\$12 + X) through to (\$15 + X) in the Base-Page,
...
The argument is encoded as a single byte that immediately follows the instruction opcode.
For example, \screentext{LDX \$12,Y} would read the value stored in location (\$12 + Y) in the Base-Page,
...
the byte immediately following the instruction opcode. The upper 8-bits are taken from the
byte following that.
For example, the instruction \screentext{LDQ \$1234}, would read the
...
For example, the instruction \screentext{LDA \$1234,X}, would read the
...
For example, the instruction \screentext{ROLQ \$1234,X}, would rotate left the 32-bit value 
...
1 cycle penalty, depending on the processor mode and speed setting.
The encoding for this addressing mode is identical to Absolute Mode.
For example, the instruction \screentext{LDA \$1234,Y}, would read the
...
then \screentext{LDA (\$12,X)} would read the contents of memory location \$1234,
...
then \screentext{LDA (\$12),Y} would read the contents of memory location \$567C (i.e., \$5678 + Y),
...
Base-Page contained \$78 and \$56, respectively,
then \screentext{LDA (\$12),Z} would read the contents of memory location \$567C (i.e., \$5678 + Z),
...
then \screentext{LDQ (\$20),Z} would read the contents of memory location \$ABD1 (i.e., \$ABCD + Y) -- \$ABD4
