...
\underline{NOTE}: Prefix instructions are executed by the CPU as if they are standalone instructions, prior to recognizing the compound instruction. This can impact flag status. For example, the instruction bytes 42 42 8D perform two NEG instructions, followed by the STQ compound instruction. The NEG instructions may impact the N and Z flags, depending on the contents of the accumulator.

You can find compound instructions highlighted in the 4510 Opcode Table. See \bookvref{sec:opctable4510}.
