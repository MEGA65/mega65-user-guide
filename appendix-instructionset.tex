...
In this modde, the argument to the instruction is a value that is used directly.
...
This addressing mode is idential to Absolute Indirect X-Indexed Mode, except that the address
...
``the Y-th byte of the memory at the address pointed to by the operand.'' That is, it dereferences
a pointer.
The encoding for this addressing mode is identical to Base Page Mode.
...
added to the address that is read from the pointer, instead of being added to the pointer.
This is a very useful mode, that is frequently because it effectively provides access to
``the Z-th byte of the memory at the address pointed to by the operand.'' That is, it dereferences
a pointer. 
The encoding for this addressing mode is identical to Base Page Mode.
...
This mode is formed by preceeding a Base Page Indirect Z-Indexed Mode instruction with
...
pointer. This addressing mode effectively dereferences a pointer that
