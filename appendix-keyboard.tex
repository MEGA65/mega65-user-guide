...
\item \$D60F[0:1]: Disambiguation bits for cursor left and cursor up; bit is set if key is pressed
\end{itemize}
...
To make it possible to distinguish between holding two cursor keys to form diagonals (such as up and left vs. up and right), the MEGA65 provides one more register, \$D60F bits 0 and 1. Bit 0 is set if cursor left is pressed, and bit 1 is set if cursor up is pressed.

All together, these registers make it possible for a program to scan for nearly every combination of keys being pressed.
