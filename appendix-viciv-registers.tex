...
\hline
\endlastfoot
\small Screen RAM byte 0 \multicolumn{1}{l|}{\small Lower 8 bits of character number, the same as the VIC-II and VIC-III }\\
\small Screen RAM byte 1, bits 0 - 4 \multicolumn{1}{l|}{\small Upper 5 bits of character number, allowing addressing of 8,192 unique characters }\\
\small Screen RAM byte 1, bits 5 - 7 \multicolumn{1}{l|}{\small Trim pixels from right side of character (bits 0 - 2) }\\
\small Colour RAM byte 0, bit 7 \multicolumn{1}{l|}{\small Vertically flip the character }\\
\small Colour RAM byte 0, bit 6 \multicolumn{1}{l|}{\small Horizontally flip the character }\\
\small Colour RAM byte 0, bit 5 \multicolumn{1}{l|}{\small Alpha blend mode (leave 0, discussed later) }\\
\small Colour RAM byte 0, bit 4 \multicolumn{1}{l|}{\small GOTO X (allows repositioning of characters along a raster via the Raster-Rewrite Buffer, discussed later) }\\
\small Colour RAM byte 0, bits 3 \multicolumn{1}{l|}{\small If set, Full-Colour characters use 4 bits per pixel and are 16 pixels wide (less right side trim bits), instead of using 8 bits per pixel, and being the usual 8 pixels wide  }\\
\small Colour RAM byte 0, bits 2 \multicolumn{1}{l|}{\small Trim pixels from right side of character (bit 3) }\\
\small Colour RAM byte 0, bits 0 - 1 \multicolumn{1}{l|}{\small Number of pixels to trim from top or bottom of character }\\
\small Colour RAM byte 1, bits 0 - 3 \multicolumn{1}{l|}{\small Low 4 bits of colour of character }\\
\small Colour RAM byte 1, bit 4 \multicolumn{1}{l|}{\small Hardware blink of character (if VIC-III extended attributes are enabled) }\\
\small Colour RAM byte 1, bit 5 \multicolumn{1}{l|}{\small Hardware reverse video enable of character (if VIC-III extended attributes are enabled)* }\\
\small Colour RAM byte 1, bit 6 \multicolumn{1}{l|}{\small Hardware bold attribute of character (if VIC-III extended attributes are enabled)* }\\
\small Colour RAM byte 1, bit 7 \multicolumn{1}{l|}{\small Hardware underlining of character (if VIC-III extended attributes are enabled) }\\
\end{longtable}
