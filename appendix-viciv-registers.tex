...
The primary innovation is the addition of the Super-Extended Attribute Mode (SEAM). The VIC-II already uses 12 bits per character: Each 8$\times$8
...
The two tables that follow describe the bitfields available for this character, which vary based on whether the \textbf{GOTOX\_flag} is set or cleared:

\begin{itemize}
  \item \textbf{GOTOX\_flag = 0} will create a \underline{visible character} (\textcolor{blue}{blue} bitfields) containing attributes impacting \underline{this character}.
  \item \textbf{GOTOX\_flag = 1} will create an \underline{invisible character} (\textcolor{red}{red} bitfields) containing attributes impacting \underline{following characters}.
\end{itemize}
...
\subsubsection{Bit Fields when GOTOX\_flag is cleared:}

\setlength{\tabcolsep}{3pt}
\begin{longtable}{|L{2.1cm}|L{9cm}|}
  \hhline{==}
  {\bf{Bit(s)}} & {\bf{Function when GOTOX\_flag is \underline{\textcolor{blue}{cleared}}}}  \\
  \hhline{==}
\endfirsthead
...
\endlastfoot
  ...
  \hline
  \small \qquad Bits 7 - 0 & {\small \textbf{\textcolor{blue}{char\_number (lo)}}: (bits 7 -- 0) Low-byte of character number\newline
  (the same as the VIC-II and VIC-III) }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 1}} \\
  \hline
  \small \qquad Bits 7 -- 5 & {\small \textbf{\textcolor{blue}{rhs\_trim (lo)}}: (bits 2 -- 0) Trim pixels from right-hand side of character)}\\
  \hline
  \small \qquad Bits 4 - 0 & {\small \textbf{\textcolor{blue}{char\_number (hi)}}: (bits 12 -- 8) Upper 5 bits of character number, allowing addressing of 8,192 unique characters }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 0}} \\
  \hline
  \small \qquad Bit 7 & {\small \textbf{\textcolor{blue}{vert\_flip}}: Vertically flip the character }\\
  \hline
  \small \qquad Bit 6 & {\small \textbf{\textcolor{blue}{horz\_flip}}: Horizontally flip the character }\\
  \hline
  \small \qquad Bit 5 & {\small \textbf{\textcolor{blue}{alpha\_mode}}: Enable alpha blend mode, pixel values are treated as alpha values blending between foreground colour and background colour (need to set \$D054.7 - ALPHEN) }\\
  \hline
  \small \qquad \textbf{Bit 4} & {\small \textbf{GOTOX\_flag} is \textbf{\underline{\textcolor{blue}{cleared}} (set to 0)} \linebreak Provides the \textcolor{blue}{blue} bitfields shown in this table, which are attributes that impact \underline{this character}. }\\
  \hline
  \small \qquad Bit 3 & {\small \textbf{\textcolor{blue}{ncm\_flag}}:
\begin{itemize}
  \item 1 = \underline{NCM char}: Uses 4 bits per pixel and is 16 pixels wide (less any right-hand side trim bits)
  \item 0 = \underline{FCM char}: Uses 8 bits per pixel and is 8 pixels wide (less any right-hand side trim bits)
\end{itemize} }\\
  \hline
  \small \qquad Bit 2 & {\small \textbf{\textcolor{blue}{rhs\_trim (hi)}}: (bit 3) Trim pixels from right-hand side of character}\\
  \hline
\small \qquad Bits 1 -- 0 & {\small RESERVED, set to 0 }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 1}} \\
  \hline
  \multicolumn{2}{|l|}{\small \quad \underline{If VIC-II multi-colour mode is enabled}: (\$D016.4 - MCM)} \\
  \hline
  \small \qquad Bits 7 -- 4 & {\small \textbf{\textcolor{blue}{colour (hi)}}: Upper 4 bits of foreground colour of character}\\
  \hline
  \multicolumn{2}{|l|}{\small \quad \underline{If VIC-III extended attributes are enabled}: (\$D031.5 - ATTR)} \\
  \hline
  \small \qquad Bit 7 & {\small \textbf{\textcolor{blue}{underline}}: Hardware underlining of character }\\
  \hline
  \small \qquad Bit 6 & {\small \textbf{\textcolor{blue}{bold}}: Hardware bold attribute of character * }\\
  \hline
  \small \qquad Bit 5 & {\small \textbf{\textcolor{blue}{reverse}}: Hardware reverse video enable of character * }\\
  \hline
  \small \qquad Bit 4 & {\small \textbf{\textcolor{blue}{blink}}: Hardware blink of character}\\
  \hline
  \multicolumn{2}{|l|}{\small \quad \underline{Remaining bit-field is common}:} \\
  \hline
\small \qquad Bits 3 -- 0 & {\small \textbf{\textcolor{blue}{colour (lo)}}: Low 4 bits of foreground colour of character }\\
\end{longtable}

% Force table to sit on a single page for more convenient reading.
\newpage
\subsubsection{Bit Fields when GOTOX\_flag is set:}

\setlength{\tabcolsep}{3pt}
\begin{longtable}{|L{2.1cm}|L{9cm}|}
  \hhline{==}
  {\bf{Bit(s)}} & {\bf{Function when GOTOX\_flag is \underline{\textcolor{red}{set}}}}  \\
  \hhline{==}
\endfirsthead
\multicolumn{2}{l@{}}{\ldots continued}\\
  \hhline{==}
...
\endlastfoot
  ...
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 0}} \\
  \hline
  \small \qquad Bits 7 - 0 & {\small \textbf{\textcolor{red}{gotox\_pos (lo)}}: (bits 7 -- 0) Lower 8 bits of new X position to start drawing \underline{following characters}, relative to the start of character drawing.  Setting to 0 causes the next character to be drawn over the top of the left-most character. }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 1}} \\
  \hline
  \small \qquad Bits 7 - 5 & {\small \textbf{\textcolor{red}{fcm\_yoffs}}: FCM Character data Y offset: When non-zero, FCM character data is fetched starting from the character address + 8 $\times$ the Y value, equivalent to one pixel line's worth of FCM character data per Y. With careful planning, this can be used to smoothly vertically scroll multiple layers of RRB content.  }\\
  \hline
  \small \qquad Bit 4 & {\small \textbf{\textcolor{red}{fcm\_yoffs\_dir}}: If set, the \textbf{\textcolor{red}{fcm\_yoffs}} field is subtracted instead of added. }\\
  \hline
\small \qquad Bits 3 - 2 & {\small RESERVED, set to 0 }\\
  \hline
  \small \qquad Bits 1 - 0 & {\small \textbf{\textcolor{red}{gotox\_pos (hi)}}: (bits 9 -- 8) Upper 2 bits of new X position (Highest bit is 2's complement signed bit) }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 0}} \\
  \hline
  \small \qquad Bit 7 & {\small \textbf{\textcolor{red}{transparent\_flag}}: If set, then background/transparent pixels will not be drawn for \underline{following characters}, allowing layering }\\
  \hline
  \small \qquad Bit 6 & {\small \textbf{\textcolor{red}{background\_flag}}: If set, \underline{following characters} will be rendered as background, allowing sprites to appear in front of them, even when sprites are set to background. }\\
  \hline
\small \qquad Bit 5 & {\small RESERVED, set to 0 }\\
  \hline
  \small \qquad \textbf{Bit 4} & {\small \textbf{GOTOX\_flag} is \textbf{\textcolor{red}{set to 1}}. Provides the \textcolor{red}{red} bitfields shown in this table, which are attributes that impact \underline{following characters}. One prominent field is \textbf{\textcolor{red}{gotox\_pos}}, discussed later in the Raster-Rewrite Buffer section).}\\
  \hline
  \small \qquad Bit 3 & {\small \textbf{\textcolor{red}{rowmask\_flag}}: If set, then the \textbf{\textcolor{red}{rowmask}} field decides which pixel rows of \underline{following characters} should be rendered. This can be used to vertically scroll characters using RRB, by drawing each character on two rows: shifted down on the top row, and shifted up on the bottom row, and masked so that only the scrolled character's pixel rows are displayed, and not data from before/after that character's data.}\\
  \hline
  \small \qquad Bit 2 & {\small \textbf{\textcolor{red}{foreground\_flag}}: If set, \underline{following characters} will be drawn as foreground, regardless of their colour, allowing sprites to appear behind them. }\\
  \hline
\small \qquad Bits 1 - 0 & {\small RESERVED, set to 0 }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 1}} \\
  \hline
  \small \qquad Bits 7 - 0 & {\small \textbf{\textcolor{red}{rowmask}}: Pixel row mask values. Bits 7 -- 0 = Rows 7 -- 0.\newline
 1 = show row, 0 = hide row }\\
\end{longtable}
