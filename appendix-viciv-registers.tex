...
\endlastfoot
  ...
  \hline
  \small \qquad bits 0 - 7 & {\small Lower 8 bits of character number, the same as the VIC-II and VIC-III }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 1}} \\
  \hline
\small \qquad bits 0 - 4 & {\small Upper 5 bits of character number, allowing addressing of 8,192 unique characters }\\
  \hline
\small \qquad bits 5 -- 7 & {\small Trim pixels from right-hand side of character (bits 0 -- 2)}\\
  \hline
...
\small \qquad bit 5 & {\small Alpha blend mode (leave 0, discussed later) }\\
  \hline
  \small \qquad \textbf{bit 4} & {\small \textbf{GOTO X is \underline{cleared} (set to 0)} \linebreak GOTOX allows repositioning of characters along a raster via the Raster-Rewrite Buffer, discussed later). Must be set to 0 for displaying characters }\\
  \hline
...
\small \qquad bit 2 & {\small Trim pixels from right-hand side of character (bit 3)}\\
  \hline
\small \qquad bits 0 -- 1 & {\small Number of pixels to trim from top or bottom of character }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 1}} \\
  \hline
\small \qquad bits 0 -- 3 & {\small Low 4 bits of colour of character }\\
  \hline
  \multicolumn{2}{|l|}{\small \quad \underline{If VIC-II multi-colour mode is enabled}:} \\
  \hline
\small \qquad bits 4 -- 7 & {\small Upper 4 bits of colour of character}\\
  \hline
...
\endlastfoot
  ...
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 0}} \\
  \hline
\small \qquad bits 0 - 7 & {\small Lower 8 bits of new X position to start drawing the next character, relative to the start of character drawing.  Setting to 0 causes the next character to be drawn over the top of the left-most character. }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Screen RAM byte 1}} \\
  \hline
\small \qquad bits 0 - 1 & {\small Upper 2 bits of new X position }\\
  \hline
\small \qquad bits 3 - 4 & {\small RESERVED, set to 0 }\\
  \hline
\small \qquad bits 5 - 7 & {\small FCM Character data Y offset: Characters display normally when set to zero. When non-zero, 8 $\times$ the value is added to the character address. With careful planning, this can be used to smoothly vertically scroll multiple layers of RRB content.  }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 0}} \\
  \hline
\small \qquad bit 7 & {\small If set, then background/transparent pixels will not be drawn for subsequent characters, allowing layering }\\
  \hline
\small \qquad bits 6 & {\small If set, the following characters will be rendered as background, allowing sprites to appear in front of them, even when sprites are set to background. }\\
  \hline
\small \qquad bit 5 & {\small RESERVED, set to 0 }\\
  \hline
  \small \qquad \textbf{bit 4} & {\small \textbf{GOTO X, set to 1} \linebreak GOTOX allows repositioning of characters along a raster via the Raster-Rewrite Buffer, discussed later). Must be set to 0 for displaying characters }\\
  \hline
...
\small \qquad bit 2 & {\small If set, the following characters will be rendered as foreground, regardless of their colouring, allowing sprites to appear behind them. }\\
  \hline
\small \qquad bits 0 - 1 & {\small RESERVED, set to 0 }\\
  \hline
  \multicolumn{2}{|l|}{\small \textbf{Colour RAM byte 1}} \\
  \hline
\small \qquad bits 0 - 7 & {\small Pixel row mask flags }\\
\end{longtable}
