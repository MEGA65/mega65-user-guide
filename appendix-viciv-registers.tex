...
\small \qquad Bit 6 & {\small Horizontally flip the character }\\
  \hline
\small \qquad Bit 5 & {\small Enable alpha blend mode, pixel values are treated as alpha values blending between foreground colour and background colour (need to set \$D054.7 - ALPHEN) }\\
  \hline
  ...
  \hline
\small \qquad Bit 3 & {\small If set:
\begin{itemize}
  \item \underline{NCM char}: Uses 4 bits per pixel and is 16 pixels wide (less any right-hand side trim bits)
\end{itemize}
If cleared:
\begin{itemize}
  \item \underline{FCM char}: Uses 8 bits per pixel and is 8 pixels wide (less any right-hand side trim bits)
\end{itemize} }\\
  \hline
