...
programme has been correctly modified. Use the \specialkey{LIST}\index{LIST}\index{BASIC 10 Commands!LIST} command again to achieve this.
  ...
  of the \screentextwide{FOR} command.\index{STEP}\index{BASIC 10 Commands!STEP}
...
\stw{IF}\index{IF}\index{BASIC 10 Commands!IF} and
\stw{THEN}\index{THEN}\index{BASIC 10 Commands!THEN} commands.
...
but just like b/efore, after one guess, it returns to the \stw{READY.}
...
\stw{RENUMBER}\index{RENUMBER}\index{BASIC 10 Commands!RENUMBER}
...
\index{STOP}\index{BASIC 10 Commands!STOP}
\index{END}\index{BASIC 10 Commands!END}
\index{CONT}\index{BASIC 10 Commands!CONT}
...
\index{FOREGROUND}\index{BASIC 10 Commands!FOREGROUND}
\index{BACKGROUND}\index{BASIC 10 Commands!BACKGROUND}
\index{BORDER}\index{BASIC 10 Commands!BORDER}}
...
{\em Clue: You will need to remember the previous guess in a different
  ...
  \stw{AND} operator.\index{AND}\index{BASIC 10 Operators!AND}}
...
following:\index{RND()}\index{BASIC 10 Functions!RND}
...
{\em Clue: You will need to modify the line that sets \stw{SN}, as
  well as the \stw{PRINT} message that gives instuction to the user.}
...
{\em Clue: To make sure you don't set the text colour to the same as
  the backgrorund, you might like to calculate which background colour
...
{\bf 4. Make the ``guess my number'' progamme randomly choose between
  two different greeting messages when it starts.}

The ``guess my number'' programe currently always prints the same
message everytime it starts.  Modify it so that it prints one of two
