...
\begin{itemize}
  ...
  \item \screentext {EXIT AND REBOOT TO ONBOARDING} abandons any changes made in the MEGA65 Configure utility and reboots to the Onboarding utility.
  \item \screentext {SAVE AS DEFAULTS AND EXIT} commits changes made to the SD card. These changes will be used when the MEGA65 is switched on.
\end{itemize}
...
\end{center}
\index{Real-Time Clock!Setting the Date/Time}
\begin{itemize}
...
\begin{itemize}
  ...
  \item \screentext{DIGITAL VIDEO}
    allows for selection between either \screentext{ENHANCED} video output containing audio,
    or \screentext{DVI ONLY} video output with no audio. \newline
  \item \screentext{CRT EMULATION}
