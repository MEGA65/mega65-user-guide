...
home computers.  We believe that the FPGA is powerful enough that it can pretend to be
...
i.e., turn off the power, then hold the \megakey{NO SCROLL} key down while turning the power on.  When the flash
...
install using the cursor keys, and then press the \megakey{RETURN} key.  The MEGA65 will then erase
