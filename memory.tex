...
This book sometimes uses a dot (.) to separate groups of four hex digits to
...
The MEGA65 provides multiple mechanisms of address translation. Memory
...
{\bf MEM}\index{BASIC Commands!MEM} command to reserve blocks in banks 4 and
...
{\bf SYS} ignores bit 7 of the {\bf BANK} setting. \texttt{BANK 128} is
...
To make it possible to access any memory location with a 16-bit address, the
...
\begin{itemize}
\item Commodore 64 ROM and I/O banking, controlled by the CPU registers at addresses 0.0000 and 0.0001
...
\bookvref{sec:addressing-modes}. For a detailed explanation of the banking
...
The offset must be a multiple of 256 bytes (\$100). The 4510 CPU from the
...
(honoring the MAP register), write the final low byte to {\bf \$D706} instead.

Addresses in the job list itself are never MAP'd addresses: DMA jobs always
operate on physical addresses. Using {\bf \$D706} only changes the way the
...
Inline job lists are always read through the CPU's view of memory, as if the
job list were CPU instructions.
...
The MEGA65 stores KERNAL and BASIC program code in banks 2 and 3. To protect
...
and KERNAL code. Naturally, this overrides the MEGA65 ROM that the user has
