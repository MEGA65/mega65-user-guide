...
Some assemblers, including the one built into the MEGA65 monitor, do not know the EOM instruction. You can substitute the NOP instruction for EOM, which uses the same encoding. Assemblers that support 65CE02 instructions use the AUG instruction for MAP.
...
Notice that the register locations are \emph{not} arranged consecutively! The MEGA65 introduces the ability to store an enhanced DMA job list at any 28-bit address. Prior to introducing this feature, there was no ``megabyte'' address byte, and programs set the ``bank'' address byte without the ``megabyte'' address byte. To maintain compatibility with these programs, writing the address bank resets the address megabyte to zero. To use all four address bytes for the enhanced DMA job list, they must be written in this unusual order.
...
Addresses in the job list itself are never MAP'd addresses: DMA jobs always operate on physical addresses. Using \$D706 only changes the way the address of the job list is interpreted, and only if it's a 16-bit address.
...
LDA #^dmajob
STA $D702
LDA #0     ; Assumes dmajob is in the first megabyte
STA $D704
LDA #>dmajob
STA $D701
LDA #<dmajob
...
The MEGA65's 45GS02 CPU extends the MAP register with the ability to access any 256 byte block in the 28-bit address space. This takes the form of an additional high byte of offset for each of MAPLO and MAPHI.
