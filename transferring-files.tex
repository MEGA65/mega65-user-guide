...
If the power light does not start blinking after pressing \specialkey{SHIFT} + \megakey{\pounds}, you may need to set DIP switch \#2 on the main board. MEGA65s manufactured in the year 2024 and later should have this switch set at the factory. Earlier MEGA65s have this switch off by default.

To set the DIP switch,\index{DIP switches} open the case, as described in chapter \vref{cha:setup}. Locate the DIP switches on the main board, then set DIP switch \#2 to the ``on'' position. Look for markings on the switches to identify switch \#2 and the ``on'' direction. (The orientation of your DIP switches may differ from this diagram.)
